<template>
    <card class="p-4 h-full">
        <div class="mb-4 border-b border-50 pb-2">
            ...
            >Upload Transaction Fee Report</h1>
            <p class="leading-normal text-center">
                <a
                    target="_blank"
                    class="text-primary no-underline hover:bg-40 py-1 px-2 rounded"
                    href="https://www.braintreegateway.com/merchants/7vm6h7d46t6vxfq8/transaction_level_fee_reports/new"
                >Go to Braintree</a>
            </p>
        </div>
...
export default {
    ...
    methods: {
        ...
        uploadFile(event) {
            ...
            axios
                .post(
                    "/nova-vendor/transactions-upload/transactions",
                    formData,
                    {
                        headers: {
                ...
                .then(function() {
                    self.busy = false;
                    self.message =
                        "File was successfully uploaded and will begin processing.";
                    document.getElementById("file").value = null;
                })
                .catch(function() {
                    self.busy = false;
                    self.message =
                        "Whoops! There was an issue uploading the file.";
                    document.getElementById("file").value = null;
                });
        }
    }
